...
class PayrocPagerTest < Minitest::Test
  def setup
    ...
    mock_response_class = Struct.new(:body)
    def @mock_client.send(request)
      mock_response_class = Struct.new(:body)
      case request.path
      when "https://api.example.com/page2"
        mock_response_class.new('{"data":[{"id":3},{"id":4}],"links":[{"rel":"next","href":"https://api.example.com/page3"},{"rel":"previous","href":"https://api.example.com/page1"}]}')
      when "https://api.example.com/page3"
        mock_response_class.new('{"data":[{"id":5}],"links":[{"rel":"previous","href":"https://api.example.com/page2"}]}')
      else
        mock_response_class.new('{"data":[]}')
      end
    end
  end
  ...
  def test_pager_with_hash_response
    ...
    assert_equal 5, pager.to_a.length
  end
